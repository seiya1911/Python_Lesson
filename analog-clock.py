...
class MyFrame(tk.Frame):
    def __int__(self, master=None):
        ...
        self.clock = tk.Canvas(self, width=self.size,
                               hight=self.size, background='white')
        self.clock.grid(row=0, column=0)

        self.font_size = int(self.size/15)
        for number in range(1, 12+1):
            x = self.size/2 + \
                math.cos(math.radians(number*360/12 - 90))*self.size/2*0.85
            y = self.size/2 + \
                math.sin(math.radians(number*360/12 - 90))*self.size/2*0.85
            self.clock.create_text(x, y, text=str(
                number), fill='black', font=("", 14))
    ...
    def display(self):
        if self.sec != time.localtime().tm_sec or self.start:
            ...
            self.clock.create_line(
                x0, y0, x, y, widht=1, fill='red', tag='SEC')
        ...
        if self.sec2 != time.localtime().tm_sec or self.toggled:
            ...
            if self.show_date:
                self.clock.create_text(x, y, text=text, font=(
